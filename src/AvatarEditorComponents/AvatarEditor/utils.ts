import { AvatarFullConfig, GenConfigFunc, Sex, EarSize, HairStyleMan, HairStyleWoman, HatStyle, EyeStyle, GlassesStyle, NoseStyle, MouthStyle, ShirtStyle, EyeBrowStyle } from './types';
...
export const genConfig: GenConfigFunc = (userConfig = {}) => {
    ...
    response.glassesStyle = isSeedConfig
        ...
        : userConfig.glassesStyle || pickRandomFromList(defaultOptions.glassesStyle, { usually: ['none'] });
    ...
    if (isSeedConfig || !userConfig.hairStyle) {
        switch (response.sex) {
            ...
            case 'woman': {
                response.hairStyle = isSeedConfig ? (pickByHashCode(hashCode, 'hairStyleWoman') as HairStyleWoman) : pickRandomFromList(defaultOptions.hairStyleWoman);
                break;
            }
        }
    } else {
    ...
    if (!isSeedConfig && userConfig.eyeBrowStyle) {
        ...
        response.eyeBrowStyle = response.sex === 'woman' ? (isSeedConfig ? (pickByHashCode(hashCode, 'eyeBrowWoman') as EyeBrowStyle) : pickRandomFromList(defaultOptions.eyeBrowWoman)) : 'up';
    }
